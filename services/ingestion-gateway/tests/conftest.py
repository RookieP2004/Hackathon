...
from aegis_db.models import Building, Plant, Role, SensorType, User, Zone
...
def _clean_test_tables():
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "TRUNCATE TABLE users, sensor_readings, sensors, weather_observations, "
                "equipment, zones, buildings, plants RESTART IDENTITY CASCADE"
            )
        )
    engine.dispose()
...
async def plant_id_factory(db_session, unique_suffix):
    ...
    async def _make() -> int:
        counter["n"] += 1
        plant = Plant(code=f"TESTPLANT-W-{unique_suffix}-{counter['n']}", name="Test Plant", timezone="UTC")
        db_session.add(plant)
        await db_session.commit()
...
@pytest_asyncio.fixture
async def sensor_type_id_factory(db_session):
    async def _make(name: str = "Vibration") -> int:
        result = await db_session.execute(select(SensorType).where(SensorType.name == name))
        existing = result.scalar_one_or_none()
        if existing:
            return existing.id
        st = SensorType(name=name, default_unit="mm/s")
        db_session.add(st)
        await db_session.commit()
