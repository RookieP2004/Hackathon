...
def _clean_test_tables():
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "TRUNCATE TABLE users, maintenance_records, risk_scores, predictions, "
                "workers, employers, equipment, zones, buildings, plants RESTART IDENTITY CASCADE"
            )
        )
    engine.dispose()
...
@pytest_asyncio.fixture
async def equipment_type_id_factory(db_session):
    async def _make(name: str = "Pump") -> int:
        result = await db_session.execute(select(EquipmentType).where(EquipmentType.name == name))
        existing = result.scalar_one_or_none()
        if existing:
            return existing.id
        et = EquipmentType(name=name, category="Rotating")
        db_session.add(et)
        await db_session.commit()
...
@pytest_asyncio.fixture
async def equipment_id_factory(db_session, unique_suffix, equipment_type_id_factory):
    """Creates a fresh Plant -> Building -> Zone -> Equipment chain, committed for real."""
    counter = {"n": 0}
    ...
    async def _make() -> int:
        ...
        equipment = Equipment(
            zone_id=zone.id, equipment_type_id=equipment_type_id,
            tag=f"EQ-{unique_suffix}-{counter['n']}", name="Test Equipment",
        )
...
@pytest_asyncio.fixture
async def maintenance_type_id_factory(db_session):
    async def _make(name: str = "Preventive") -> int:
        result = await db_session.execute(select(MaintenanceType).where(MaintenanceType.name == name))
        existing = result.scalar_one_or_none()
        if existing:
            return existing.id
        mt = MaintenanceType(name=name)
        db_session.add(mt)
        await db_session.commit()
