...
from aegis_db.models import Building, EquipmentType, Plant, Role, User, Zone
...
def _clean_test_tables():
    ...
    with engine.begin() as conn:
        conn.execute(text("TRUNCATE TABLE users, equipment, zones, buildings, plants RESTART IDENTITY CASCADE"))
    engine.dispose()
...
async def zone_id_factory(db_session, unique_suffix):
    """Creates a fresh Plant -> Building -> Zone chain, committed for real, so
    machine tests have a real zone_id to reference without depending on any
    other service's test suite or seed data having already run."""
    counter = {"n": 0}
