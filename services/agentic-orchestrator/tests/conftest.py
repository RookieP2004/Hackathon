...
from aegis_db.models import Plant, Role, User
...
def _clean_test_tables():
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                "TRUNCATE TABLE users, emergency_event_steps, emergency_events, "
                "playbook_steps, playbooks, plants RESTART IDENTITY CASCADE"
            )
        )
    engine.dispose()
